...
# 所有测试物种（包括 Apple 自身和 Cherry 修正名）
TEST_SPECIES = [
    "Apple",            # 同物种对照
    "Corn_(maize)",
    "Tomato",
...
# ============================================
# 3. 构建健康基准库 (Apple 健康图像的一部分)
# ============================================
print(f"\n构建健康基准库 (物种: Apple)...")
apple_healthy_paths = dataset.get("Apple", {}).get("healthy", [])
if len(apple_healthy_paths) == 0:
    raise RuntimeError("数据集中没有 Apple/healthy 图像！")
print(f"Apple 健康图像总数: {len(apple_healthy_paths)}")

# 随机选择 MAX_BENCHMARK_IMAGES 张作为基准
np.random.seed(42)
if len(apple_healthy_paths) > MAX_BENCHMARK_IMAGES:
    benchmark_paths = np.random.choice(apple_healthy_paths, MAX_BENCHMARK_IMAGES, replace=False).tolist()
else:
    benchmark_paths = apple_healthy_paths
print(f"选用 {len(benchmark_paths)} 张作为健康基准库")

print("提取健康基准特征...")
all_healthy_features = []
for i, path in enumerate(benchmark_paths):
    if (i + 1) % 10 == 0:
        print(f"  进度: {i+1}/{len(benchmark_paths)}")
    try:
...
# ============================================
print(f"\n{'='*60}")
print(f"跨物种零样本异常检测 (完整版)")
print(f"基准物种: Apple (30张健康)")
print(f"{'='*60}")

all_results = {}
# 用于记录所有测试物种的完整列表（包括无结果的那些）
tested_species_list = []
...
for test_species in TEST_SPECIES:
    ...
    # ---------- 特殊处理 Apple 同物种测试 ----------
    if test_species == "Apple":
        # 剩余的健康图像
        used_benchmark_set = set(benchmark_paths)
        rest_healthy = [p for p in apple_healthy_paths if p not in used_benchmark_set]
        # 限制数量
        if len(rest_healthy) > MAX_TEST_PER_CONDITION:
            rest_healthy = np.random.choice(rest_healthy, MAX_TEST_PER_CONDITION, replace=False).tolist()
        # 病害图像
        disease_paths = []
        for cond in ["Apple_scab", "Black_rot", "Cedar_apple_rust"]:
            disease_paths.extend(dataset["Apple"].get(cond, [])[:MAX_TEST_PER_CONDITION])
        test_paths = rest_healthy + disease_paths
        test_labels = [0]*len(rest_healthy) + [1]*len(disease_paths)
        print(f"\n--- 同物种测试: Apple ---")
        print(f"  测试图像: {len(test_paths)} (健康: {len(rest_healthy)}, 病害: {len(disease_paths)})")
    else:
        # 其他物种
        test_paths = []
        test_labels = []
...
print(f"基准物种: Apple (30张健康)")
print(f"{'='*60}")
print(f"{'物种':<30} {'样本':<8} {'几何AUROC':<12} {'拓扑AUROC':<12} {'融合AUROC':<12}")
print("-" * 80)

for species in tested_species_list:
    if species in all_results:
        res = all_results[species]
        print(f"{species:<30} {res['n_test']:<8} {res['geo_auroc']:.4f}        {res['topo_auroc']:.4f}        {res['fusion_auroc']:.4f}")
    else:
        print(f"{species:<30} {'N/A':<8} {'N/A':<12} {'N/A':<12} {'N/A':<12}")
...
if all_results:
    ...
    print("-" * 80)
    print(f"{'平均':<30} {'':<8} {avg_geo:.4f}        {avg_topo:.4f}        {avg_fusion:.4f}")
...
if all_results:
    ...
    ax.bar(x - width, geo_aurocs, width, label='Geometric Score', color='#3498db')
    ax.bar(x, topo_aurocs, width, label='Topological Score', color='#e74c3c')
    ax.bar(x + width, fusion_aurocs, width, label='Fusion Score', color='#2ecc71')
    
    ax.set_ylabel('AUROC')
    ax.set_title(f'Zero-Shot Cross-Species Anomaly Detection (Benchmark: Apple, 30 healthy)')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig("topoleaf_complete_cross_species.png")
    print("\n完整结果图已保存: topoleaf_complete_cross_species.png")